<script>
    ...
    async function checkWatchlistStatus() {
        if (!pb || !userStore.user || !itemId) return;
        
        try {
            const existing = await pb.collection('watchlist').getFirstListItem(
                `userId = "${userStore.user.id}" && itemId = "${itemId}"`
            ).catch(() => null);
                ...
                if (existing) {
                    ...
                    try {
                        ...
                        await pb.collection('items').update(itemId, {
                            watchCount: Math.max(0, (item.watchCount || 1) - 1)
                        });
                    } catch (itemErr) {
